 ...
 * @author    Newfangled
 * @link      https://bitbucket.org/newfangled_web/newfangled-gravityforms-acton
 * @copyright Newfangled 2016
 */

class RGActOnUpgrade{

    public static function set_version_info($version_info){
        if ( function_exists('set_site_transient') )
            set_site_transient("gforms_acton_version", $version_info, 60*60*12);
        else
            set_transient("gforms_acton_version", $version_info, 60*60*12);
    }
    ...
    public static function display_changelog($offering, $key, $version){
        ...
        if ( is_wp_error( $raw_response ) || 200 != $raw_response['response']['code']){
            $page_text = sprintf(__("Oops!! Something went wrong.%sPlease try again or %scontact us%s.", 'gravityformsacton'), "<br/>", "<a href='http://www.gravityforms.com'>", "</a>");
        }
        else{
    ...
    public static function get_version_info($offering, $key, $version, $use_cache=true){
        ...
        $version_info = function_exists('get_site_transient') ? get_site_transient("gforms_acton_version") : get_transient("gforms_mailchimp_version");
