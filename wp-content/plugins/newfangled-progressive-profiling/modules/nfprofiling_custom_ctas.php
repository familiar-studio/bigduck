...
/**
 * Gravity Custom CTA (custom post type) progressive profiling module
 *
 ...
 * as possible progressive profiling fields. 
 *
 * CTAs are pulled from a pool of custom post types, called
 * 'Sidebar CTAs'. These custom posts in turn display a 
 * Gravity Form. Smart CTA logic dictates that if the visitor
 * has every completed that Smart CTA's form (anywhere on the site)
 * then that Smart CTA is considered 'completed', and the next one shows.
*/
class NfProfiling_CustomCTAs extends NfProfiling_Module
{
    var $modulename = 'Custom Sidebar CTAs';
    var $moduleid   = 'customctas';
    var $moduledesc = 'CTAs are pulled from a pool of custom post types, called \'Sidebar CTAs\'. These custom posts in turn display a Gravity Form. Smart CTA logic dictates that if the visitor has every completed that Smart CTA\'s form (anywhere on the site)then that Smart CTA is considered \'completed\', and the next one shows. <b>Note</b>: for this module to work, the CTA post type slug must be called \'sidebarcta\', and must contain an ACF field called \'cta_form\', which returns the Gravity Form object.';
   ...
   function prepare_fields($form) {
    ...
    function store_submission($entry, $form){
        ...
        if ($user_email_value)
        {
            ...
            if ($form['fields'])
            {
                foreach( $form['fields'] as $field )
                {
                    if ($field['inputName'] == 'cta_id')
                    {
                        if ( $entry[ $field['id'] ] )
                        {
                            $cta_id = $entry[ $field['id'] ];
                        }
                    }
                }
            }
    ...
    /**
     * Get the CTA items that are available as
     * Smart CTAs.
     *
    ...
    function get_smartcta_forms( $ignore_excluded=true ){
        ...
        // $cache_key = print_r($args,1);

        // if (!$ctas = wp_cache_get( $cache_key ))
        // {
        //     $ctas = get_posts( $args );
        //     wp_cache_set( $cache_key, $ctas, '', 600 );
        // }
            ...
            if($ctas)
            {
                ...
                {
                    if ($ignore_excluded) {
                        if (isset($this->parent->settings->options['smartcta_excludeform_' . $cta->ID])) {
                            continue;
                        }
                    }
                    
                    $post = get_post($cta->ID); 
                    setup_postdata( $post );
                    
                    $desc = 'Post ID: ' . $cta->ID;

                    if ($related_form = get_field( 'cta_form', $cta->ID ) ) {
                        $desc .= ' | Related Form: ' . $related_form['title'];
                    }

                    $form_parms                 = Array();
                    $form_parms['module']       = $this->moduleid;
                    $form_parms['moduledesc']   = $this->modulename;
                    $form_parms['id']           = get_the_ID();
                    $form_parms['name']         = get_the_Title();
                    $form_parms['details']      = $desc;
    ...
    function render_form( $formid, $formname ){
        ...
        if (file_exists(get_stylesheet_directory() . '/nfprofiling-smartcta-customcta.php' )) {

            include( get_stylesheet_directory() . '/nfprofiling-smartcta-customcta.php' );

        } else { 

            include( NFPROFILING_DIR . '/templates/nfprofiling-smartcta-customcta.php' );
        }
     ...
     * @return boolean
     */
    function has_cta_been_submitted( $cta_id ){
...
array_push( $nfprofiling_moduleclasses, 'NfProfiling_CustomCTAs' );
